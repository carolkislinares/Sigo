...
namespace Nop.Web.Controllers
{
    public partial class OrderController : BasePublicController
    {
        ...
        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult RegisterPayment(int orderId)
        {
            var order = _orderService.GetOrderById(orderId);
            if (order == null || order.Deleted || _workContext.CurrentCustomer.Id != order.CustomerId)
                return Challenge();
            ...
            var model = _orderModelFactory.PrepareOrderDetailsModel(order);
            model.VatNumber = "PaymentRegisterTransfer";
            return View("PaymentRegister", model);
           // return RedirectToAction("RegisterPayment", "PaymentTransfer", new { id = orderId, Area = "Admin" });

        }

        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult RegisterPaymentZelle(int orderId)
        {
            ...
            var model = _orderModelFactory.PrepareOrderDetailsModel(order);
            model.VatNumber = "PaymentRegisterZelle";
            return View("PaymentRegister", model);
