...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        protected virtual void SetOrderStatus(Order order, OrderStatus os, bool notifyCustomer)
        {
            ...
            AddOrderNote(order, $"El estado del pedido ha cambiado a { os.ToString()}");
        ...
        public virtual void MarkOrderAsPaid(Order order)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                throw new NopException("No puede marcar este pedido como pagado");
            ...
            //add a note
            AddOrderNote(order, "La orden ha sido marcada como pagada.");
            ...
            if (order.PaymentStatus == PaymentStatus.Paid)
            {
                 ...
                 if(order.ShippingAddressId!=null && order.PickupAddressId == null)
                {
                    ...
                    foreach (var orderItem in order.OrderItems)
                    {
                        ...
                        if (shipment == null)
                        {
                            ...
                            var adminComment = string.Format("Envío generado para la orden #",order.Id);
                    ...
                    if (shipment != null && shipment.ShipmentItems.Any())
                    {
                        ...
                        AddOrderNote(order, "Envío generado automáticamente.");     
                    }
                }
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            if (!CanRefund(order))
                throw new NopException("No se puede reembolsar el pedido.");
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    //add a note    
                    AddOrderNote(order, $"El pedido ha sido reembolsado. Monto = {request.AmountToRefund}");
                    ...
                    if (orderRefundedStoreOwnerNotificationQueuedEmailIds.Any())
                        AddOrderNote(order, $"\"Pedido reembolsado\" correo electrónico (al administrador de la tienda) se ha puesto en cola. Identificadores de correo electrónico en cola: {string.Join(", ", orderRefundedStoreOwnerNotificationQueuedEmailIds)}.");
                    ...
                    if (orderRefundedCustomerNotificationQueuedEmailIds.Any())
                        AddOrderNote(order, $"\"Pedido reembolsado\"correo electrónico (al cliente) se ha puesto en cola. Identificadores de correo electrónico en cola: {string.Join(", ", orderRefundedCustomerNotificationQueuedEmailIds)}.");
            ...
            //add a note
            AddOrderNote(order, $"No se puede reembolsar el pedido. {error}");

            //log it
            var logError = $"Error orden de reembolso #{order.Id}. Error: {error}";
            _logger.InsertLog(LogLevel.Error, logError, logError);
            return result.Errors;
        }
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                throw new NopException("No puede reembolsar este pedido");
            ...
            //add a note
            AddOrderNote(order, $"El pedido ha sido marcado como reembolsado. Monto = {amountToRefund}");
        ...
        public virtual IList<string> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefund(order, amountToRefund))
                throw new NopException("No se puede hacer un reembolso parcial por pedido.");
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    //add a note
                    AddOrderNote(order, $"El pedido ha sido parcialmente reembolsado. Monto = {amountToRefund}");
